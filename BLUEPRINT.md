...
Paper labels in this document follow the public arXiv manuscript: Theorem 1,
Corollary 1, Proposition 1, Remark 1, Lemmas 1--5, Proposition 2, and equations
(1)--(2). Lean declaration names retain their established internal numbering
...
The paper's Lemma 2 is certified by `lemma_4_2_hermite_sign`; it proves the
positive derivative identity used to rule out a three-distinct-coordinate
local maximum. Paper Lemma 3 is certified by
`lemma_4_3_three_coordinate`, which packages the constrained
three-coordinate conclusion. Paper Proposition 2 is certified by
`proposition_4_4_two_level`, giving a global maximizer on

\[
\left\{y\in\mathbb R^N:
\sum_i y_i=0,\ \|y\|_2^2=\rho^2\right\}
\]
...
The paper's Lemma 4 is certified by `lemma_4_5_hypergeometric`, which proves
...
`theorem_2_1_upperTail` proves the arbitrary-threshold form

\[
\Pr\{Y\ge u\}
\le \exp\!\left\{-\frac{u^2}
{2\Gamma_N\|P_{\mathbf1^\perp}\widetilde w\|_2^2}\right\}.
\]

Substituting

\[
u=\|P_{\mathbf1^\perp}\widetilde w\|_2
...
direct kernel dependencies of `theorem_2_1_mgf`. The stronger certificate
...
Paper Proposition 1 is certified by `proposition_2_3`, which proves
...
Paper Remark 1 is certified by `remark_2_4_variational`, which identifies
...
Paper Lemma 5 is certified by `lemma_4_6`, which proves
\(\Gamma_N<1+\epsilon_N\) for \(N\ge3\), and by `lemma_4_6_eq_two`, which
...
1. **Statement pass:** `centeredWeight` → `Gamma` → `theorem_2_1_mgf` →
   `corollary_2_2`.
2. **Mechanism pass:** `lemma_4_1_hoeffding` → `lemma_4_2_hermite_sign` →
   `lemma_4_3_three_coordinate` → `proposition_4_4_two_level` →
   `lemma_4_5_hypergeometric`.
3. **Sharpness pass:** `Gamma_eq_lastTerm` → parity closed forms →
   `Gamma_asymptotic` → `lemma_4_6` → `proposition_2_3` →
   `remark_2_4_variational`.
