...
/-- The inverse-square tail appearing in Lemma 4.5, indexed from the
right endpoint: `j = 0` is the term `(N-1)⁻²`. -/
noncomputable def inverseSquareTail (N s : ℕ) : ℝ :=
  ∑ j ∈ Finset.range s, 1 / (((N - 1 - j : ℕ) : ℝ) ^ 2)

/-- The martingale coefficient `B_{N,m}` of Lemma 4.5. -/
...
/-- The inflation factor `Γ_N` from Theorem 2.1, as its literal finite
