...
This audit is tied to the public paper draft with SHA-256
`a7d8d51a32c619f117eb127a9a3c30b616234c0a37a4cc4c3ba44a15795a1e78`.
All unqualified declarations below lie in
`SharpSerfling.ExchangeableHoeffding`.
...
| \(\|P_{\mathbf1^\perp}\widetilde w\|_2^2\) | `sqNorm (centeredWeight hn w)` | squared Euclidean norm of the centered weight vector |
...
Paper labels below follow the public arXiv manuscript. The Lean declarations
retain established internal identifiers whose numerals need not match the
paper labels.

| Paper item | Lean declaration(s) | Exact formal content |
|---|---|---|
| Theorem 1 | `theorem_2_1_mgf`, `Gamma_isCoefficient` | logarithmic MGF inequality; also an affine-normalized strengthening over every interval \([a,b]\) |
| One-sided exponential form | `theorem_2_1_upperTail` | Chernoff bound at an arbitrary positive threshold |
| Corollary 1 | `corollary_2_2` | confidence-parameter form for \(0<\delta<1\), with the necessary hypothesis \(P_{\mathbf1^\perp}\widetilde w\ne0\) for the displayed weak-tail event |
...
| Lemma 5 | `lemma_4_6`, `lemma_4_6_eq_two` | strict inequality for \(N\ge3\), equality for \(N=2\) |
| Proposition 1 | `varianceLowerConstant`, `varianceLowerConstant_le_exact`, `proposition_2_3` | parity-dependent lower bound for every valid uniform coefficient |
| Remark 1 | `paperNormalizedLogMgf`, `variationalConstant`, `remark_2_4_variational` | literal paper normalization and equality with the exact optimal coefficient |
| Lemma 1 | `lemma_4_1_hoeffding` | Hoeffding's lemma on an arbitrary probability space |
| Lemma 2 | `lemma_4_2_hermite_sign` | positive Hermite weighted-derivative sum for three ordered roots |
| Lemma 3 | `lemma_4_3_three_coordinate` | every global constrained maximizer has a repeated coordinate |
| Proposition 2 | `proposition_4_4_two_level` | existence of a slice-MGF maximizer with at most two coordinate values |
| Lemma 4 | `sampleVarianceFactor_le_martingaleFactor`, `lemma_4_5_hypergeometric` | centered hypergeometric log-MGF bound with the paper's \(B_{N,m}\) |
...
weighted_exchangeable_mgf_centeredNorm_inLaw
  ...
  → theorem_2_1_mgf
  → theorem_2_1_upperTail
  → corollary_2_2
```

Thus the structural lemmas are not falsely presented as direct kernel
dependencies of `theorem_2_1_mgf`. They formalize the paper-facing proof
